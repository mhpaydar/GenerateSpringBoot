...
public class DTConnection {
    ...
    public String getPkName(String schemaName, String tableName) {
        if (dbType.equals(DbType.ORACLE)) {
            return "select tc.COLUMN_ID,\n"
                    + "                      tc.COLUMN_NAME,\n"
                    + "                      tc.DATA_TYPE,\n"
                    + "                      nvl(tc.DATA_PRECISION, tc.DATA_LENGTH) as len,\n"
                    + "                      tc.NULLABLE,\n"
                    + "                      tc.DATA_DEFAULT,\n"
                    + "                      tc.TABLE_NAME,\n"
                    + "                      (select cc.COMMENTS\n"
                    ...
                    + "                          and cc.COLUMN_NAME = tc.COLUMN_NAME) as comm,\n"
                    + "                      'yy' as java_type,\n"
                    + "                      decode(tc.nullable, 'N', 0, 1) as java_null,\n"
                    + "                        nvl(tc.DATA_SCALE, 0) as scale,\n"
                    + "                        '' as extra\n"
                    + "                 from all_tab_cols tc\n"
                    + "                where owner = '" + schemaName.toUpperCase() + "'\n"
                    + "                  and tc.table_name = '" + tableName.toUpperCase() + "'\n"
                    + "                  and tc.COLUMN_NAME in\n"
                    + "                      (select ic.COLUMN_NAME\n"
                    + "                         from all_constraints c,\n"
                    + "                              all_indexes     i,\n"
                    + "                              all_ind_columns ic\n"
                    + "                        where c.OWNER = '" + schemaName.toUpperCase() + "' \n"
                    + "                          and i.TABLE_NAME = '" + tableName.toUpperCase() + "' \n"
